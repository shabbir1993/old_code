...
describe SalesOrdersController do
  ...
  context "user session" do
    ...
    describe "#index" do
      ...
      let(:filtered_sales_orders) { double.as_null_object }
      before do
        expect(sales_orders).to receive_message_chain(:send, :filter, :by_code) { filtered_sales_orders }
        expect(filtered_sales_orders).to receive(:page).with("2") { paged_sales_orders }
        get :index, page: 2, status: "in_progress"
      end
    ...
    describe "#create" do
      it "creates a new tenant sales_order when valid" do
        expect(new_sales_order).to receive(:save) { new_sales_order }
        xhr :post, :create, sales_order: {}, format: :js
        expect(response).to render_template(:create)
      end

      it "displays error messages when invalid" do
        expect(new_sales_order).to receive(:save) { false }
        xhr :post, :create, sales_order: {}, format: :js
    ...
    describe "#update" do
      ...
      it "updates tenant sales_order when valid" do
        expect(sales_order).to receive(:update_attributes).with(attrs) { sales_order }
        xhr :patch, :update, id: 1, sales_order: attrs, format: :js
        expect(response).to render_template(:update)
      end

      it "displays error messages when invalid" do
        expect(sales_order).to receive(:update_attributes).with(attrs) { false }
        xhr :patch, :update, id: 1, sales_order: attrs, format: :js
        expect(response).to render_template(:display_error_messages)
      end
    end
