class MasterFilmsController < ApplicationController
  ...
  def master_film_params
    params.require(:master_film).reverse_merge(defects: {}).permit(:serial, :effective_width, :effective_length, :formula, :mix_mass, :film_code, :machine_id, :thinky_code, :chemist, :operator, :inspector, :note, :micrometer_left, :micrometer_right, :run_speed, :function).tap do |white_listed|
