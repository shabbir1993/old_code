class YieldChart
  ...
  def initialize(tenant, inputs)
    @master_films_with_yield = tenant.widgets(MasterFilm).active.in_house.formula_like(inputs[:formula]).date_range(inputs[:start_date].to_date, inputs[:end_date].to_date).by_serial.reverse.reject { |mf| mf.yield.nil? }
  end
